            ...
            case REQUEST_CODE_CROP:
                ...
                if(croppedImageUri != null) {
                    ...
                    try (FileInputStream fis = openFileInput("user_id");
                        ...
                        // Upload the image uri to Firebase Storage, the root referrence of which is
                        // the user id generated by Firestore.
                        StorageReference storageRef = storage.getReference();
                        StorageReference imageRef = storageRef.child("images");
                        StorageReference profileRef = imageRef.child(userId + "/profile.jpg");

                        log.i("ProfileImage path and name: %s, %s", profileRef.getPath(), profileRef.getName());

                        profileRef.putFile(croppedImageUri)
                                .addOnProgressListener(listener -> {
                                    log.i("Progress: %s", listener);
                                })
